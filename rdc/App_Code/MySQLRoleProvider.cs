...
namespace Andri.Web
{
    ...
    public sealed class MySqlRoleProvider : RoleProvider
    {
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            //Bloco abaixo prepara string sem o metadata - TNM 03/05/2013
            if (connectionString.IndexOf("metadata")>=0)
            {
                /* Procura a posição inicial da string "server=" */
                int posini = connectionString.IndexOf("server=");

                /* Imprime a partir de server até o final" */
                connectionString = connectionString.Substring(posini);
